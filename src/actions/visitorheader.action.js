...
import {
  HTTP_VISITORHEADER_SUCCESS,
  HTTP_VISITORHEADER_FETCHING,
  HTTP_VISITORHEADER_FAILED,
  HTTP_VISITORHEADER_CLEAR,
  server,
} from "../constants";

export const setStateVisitorHeaderToSuccess = (payload) => ({
  type: HTTP_VISITORHEADER_SUCCESS,
  payload,
});
...
const setStateVisitorHeaderToFetching = () => ({
  type: HTTP_VISITORHEADER_FETCHING,
});

const setStateVisitorHeaderToFailed = () => ({
  type: HTTP_VISITORHEADER_FAILED,
});
...
const setStateVisitorHeaderToClear = () => ({
  type: HTTP_VISITORHEADER_CLEAR,
});
...
export const addVisitorHeader = (formData, history) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(setStateVisitorHeaderToSuccess(result.data));
      // alert(JSON.stringify(result.data));
      return result.data;
    } catch (err) {
      dispatch(setStateVisitorHeaderToFailed());
      alert(err.message);
    }
  };
};
