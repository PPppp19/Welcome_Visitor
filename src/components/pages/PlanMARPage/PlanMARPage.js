...
export default (props) => {
  ...
  useEffect(() => {
    ...
    marheads.map((item) => {
      ...
      handleCheckCOOApprove(item.COSTCENTER, item.MHREM2);
      handleCheckCEOApprove(item.MHBU, item.MHREM2);
  ...
  useEffect(() => {
    ...
    let checkWHS = handleCheckWHSApprove(mardetails);
    handleCheckSubmit(mardetails);

    if (checkWHS) {
      setMARHead({ ...marhead, vAppWHS: initialStateMARHead.vAppWHS });
    } else {
      setMARHead({ ...marhead, vAppWHS: "" });
    }
  }, [mardetailReducer]);
  ...
  const handleCheckCOOApprove = (costcenter, purpose) => {
    // console.log(costcenter);
    // console.log(purpose);
    let getFullCostcenter = costcenter.split(":");
  ...
  const handleCheckCEOApprove = (bu, purpose) => {
    // console.log(bu);
    // console.log(purpose);
    if (bu === "BU02" && purpose === "Adj. pro-rata import") {
      setCEOApprove(true);
      return true;
    } else {
      setCEOApprove(false);
                  ...
                  <TextField
                    ...
                    onChange={(event) => {
                      ...
                      if (event.target.value) {
                        ...
                        let checkCOO = handleCheckCOOApprove(
                          event.target.value,
                          marhead.vPurpose
                        );
                ...
                <Grid item xs={12} md={2}>
                  <TextField
                    ...
                    onChange={(event) => {
                      ...
                      let checkCEO = handleCheckCEOApprove(
                        marhead.vBU,
                        event.target.value
                      );
              ...
              editable={{
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    ...
                    // console.log("newData.ONHAND: " + newData.ONHAND);
                    // console.log("oldData.MLSTQT: " + oldData.MLSTQT);

                    if (
                      parseFloat(newData.ONHAND) > 0 &&
                      parseFloat(newData.ONHAND) <= parseFloat(oldData.MLSTQT)
                    ) {
                      console.log("true");
                      ...
                      let checkWHS = handleCheckWHSApprove([...dataUpdate]);
                      handleCheckSubmit([...dataUpdate]);

                      if (checkWHS) {
                        setMARHead({
                          ...marhead,
                          vAppWHS: initialStateMARHead.vAppWHS,
                        });
                      } else {
                        setMARHead({ ...marhead, vAppWHS: "" });
                      }
        ...
        <Grid container item xs spacing={1}>
          ...
          <Grid item xs={6} md={2}>
            <TextField
              ...
              values={(values.vApprove4 = marhead.vApprove4)}
              onChange={(event) => {
                // console.log(event.target.value);
                setMARHead({
                  ...marhead,
          ...
          {whsapprove ? (
            <Grid item xs={6} md={2}>
              <TextField
                ...
                id="vAppWHS"
                label="WHS Manager"
                value={marhead.vAppWHS}
                values={(values.vAppWHS = marhead.vAppWHS)}
                // value={initialStateMARHead.vAppWHS}
                // values={(values.vAppWHS = initialStateMARHead.vAppWHS)}
                onChange={(event) => {
                  // console.log(event.target.value);
                  setMARHead({
                    ...marhead,
                    vAppWHS: event.target.value,
                  });
                }}
                InputLabelProps={{ shrink: true }}
                SelectProps={{
          ...
          {ceoapprove ? (
            <Grid item xs={6} md={2}>
              <TextField
                ...
                values={(values.vApprove5 = marhead.vApprove5)}
                onChange={(event) => {
                  // console.log(event.target.value);
                  setMARHead({
                    ...marhead,
            ...
            (rowData) => ({
              ...
              onClick: (event, rowData) => {
                ...
                window.open(
                  ...
                  }/mar_api/data/file/${rowData.MFPREF +
                    "-" +
                    rowData.MFORNO}/${rowData.MFLINE}/${
                    rowData.MFNAME
                  }/${loginActions.getToken()}`,
    ...
    <div className={classes.root}>
      ...
      <Formik
        ...
        onSubmit={(values, { setSubmitting }) => {
          ...
          values.vPrefix = marhead.vPrefix;
          values.vApprove1 = marhead.vApprove1;
          values.vApprove2 = marhead.vApprove2;
          values.vApprove3 = marhead.vApprove3;
          values.vApprove4 = marhead.vApprove4;
          values.vApprove5 = marhead.vApprove5;
          values.vAppWHS = marhead.vAppWHS;
          ...
          if (values.vSubmit === "allocate") {
            ...
            (async function() {
              let data = await dispatch(
                ...
                // marheadActions.confirmMAR(values.vMARNumber, values.vSubmit)
              );
              alert(JSON.stringify(data));
      ...
      <MaterialTable
        id="root_mar"
        title={`MAR-D Number : ${marhead.vStatus}`}
            ...
            new Promise((resolve, reject) => {
              ...
              data.map((item) => {
                ...
                if (
                  oldData.MLRENO === ""
                  // ||
                  // oldData.MGTRSL.substring(0, 1) === "1" ||
                  // oldData.MGTRSL.substring(0, 1) === "2"
                ) {
                  (async function() {
                    await dispatch(
                    ...
                    let checkWHS = handleCheckWHSApprove([...dataDelete]);
                  ...
                  alert(
                    `This item have order in M3, Please canel order ${oldData.MLRENO} before delete item.`
                  );
                }
              });
              resolve();
